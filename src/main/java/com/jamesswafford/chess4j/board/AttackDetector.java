...
public final class AttackDetector {
    ...
    public static boolean attackedByKing(Board board, Square sq, Color player) {
        return (Bitboard.kingMoves[sq.value()] & Bitboard.squares[board.getKingSquare(player)
            .value()]) != 0;
    }
    ...
    public static boolean attackedByQueen(Board board, Square sq, Color player) {
        return (Magic
            .getQueenMoves(board, sq.value(), player == Color.WHITE ? board.getWhiteQueens() : board.getBlackQueens()))
            != 0;
    }

    public static boolean attackedByRook(Board board, Square sq, Color player) {
        return (Magic
            .getRookMoves(board, sq.value(), player == Color.WHITE ? board.getWhiteRooks() : board.getBlackRooks()))
            != 0;
    }
    ...
    public static long getAttackers(Board board, Square sq, Color color) {
        ...
        attackers |= Bitboard.kingMoves[sqVal] & Bitboard.squares[board.getKingSquare(color)
            .value()];
        attackers |= Magic
            .getRookMoves(board, sqVal, (color == Color.WHITE ? board.getWhiteRooks() : board.getBlackRooks()));
        attackers |= Magic
            .getBishopMoves(board, sqVal, (color == Color.WHITE ? board.getWhiteBishops() : board.getBlackBishops()));
        attackers |= Magic
            .getQueenMoves(board, sqVal, (color == Color.WHITE ? board.getWhiteQueens() : board.getBlackQueens()));
