        ...
        if (GameStatus.CHECKMATED.equals(gs)) {
            if (b.getPlayerToMove()
                ...
                ChessEngineProxy.getInstance()
                    .publishAnswer("RESULT 0-1 {Black mates}\n");
            } else {
                ChessEngineProxy.getInstance()
                    .publishAnswer("RESULT 1-0 {White mates}\n");
            }
        } else if (GameStatus.STALEMATED.equals(gs)) {
            ChessEngineProxy.getInstance()
                .publishAnswer("RESULT 1/2-1/2 {Stalemate}\n");
        } else if (GameStatus.DRAW_MATERIAL.equals(gs)) {
            ChessEngineProxy.getInstance()
                .publishAnswer("RESULT 1/2-1/2 {Draw by lack of mating material}\n");
        } else if (GameStatus.DRAW_BY_50.equals(gs)) {
            ChessEngineProxy.getInstance()
                .publishAnswer("RESULT 1/2-1/2 {Draw by 50 move rule}\n");
        } else if (GameStatus.DRAW_REP.equals(gs)) {
            ChessEngineProxy.getInstance()
                .publishAnswer("RESULT 1/2-1/2 {Draw by repetition}\n");
        }
    }
}
