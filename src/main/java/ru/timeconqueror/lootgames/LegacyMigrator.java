...
public class LegacyMigrator {
    ...
    private static void migrateConfigs(LegacyLGConfig legacyCfg) {
        ...
        catWorldGen.get(ConfigGeneral.Names.DISABLE_DUNGEON_GEN)
            .set(!legacyCfg.WorldGenEnabled);
        catWorldGen.get(ConfigGeneral.Names.DUNGEON_LOG_LEVEL)
            .set(legacyCfg.DungeonLoggerLogLevel);
        catWorldGen.get(ConfigGeneral.Names.PER_DIMENSION_CONFIGS)
            .set(
                legacyCfg.DimensionWhitelist.entrySet()
                    .stream()
                    .map(e -> e.getKey() + "|" + e.getValue())
                    .toArray(String[]::new));

        ConfigCategory catMain = cfgGeneral.getCategory(ConfigGeneral.Names.CATEGORY_MAIN);
        catMain.get(ConfigGeneral.Names.DISABLE_MINIGAMES)
            .set(!legacyCfg.MinigamesEnabled);
        ...
        catGol.get(ConfigGOL.Names.START_DIGIT_AMOUNT)
            .set(legacyGol.StartDigits);
        catGol.get(ConfigGOL.Names.ATTEMPT_COUNT)
            .set(legacyGol.MaxGameTries);
        catGol.get(ConfigGOL.Names.EXPAND_FIELD_AT_STAGE)
            .set(legacyGol.ExpandPlayFieldAtStage);
        catGol.get(ConfigGOL.Names.TIMEOUT)
            .set(legacyGol.Timeout);
        catGol.get(ConfigGOL.Names.EXPLODE_ON_FAIL)
            .set(legacyGol.GameFail_Explode);
        catGol.get(ConfigGOL.Names.ZOMBIES_ON_FAIL)
            .set(legacyGol.GameFail_Spawn);
        catGol.get(ConfigGOL.Names.LAVA_ON_FAIL)
            .set(legacyGol.GameFail_Lava);
        ...
        rounds = processStageConfig(
            ...
            cfgRewards.getCategory(
                LGConfigs.REWARDS.rewardsGol.getStage1()
                    .getCategoryName()),
            cfgRewards.getCategory(
                LGConfigs.REWARDS.rewardsMinesweeper.getStage1()
                    .getCategoryName()),
            cfgRewards.getCategory(
                LGConfigs.REWARDS.rewardsSudoku.getStage1()
                    .getCategoryName()));
        rounds = processStageConfig(
            rounds,
            ...
            cfgRewards.getCategory(
                LGConfigs.REWARDS.rewardsGol.getStage2()
                    .getCategoryName()),
            cfgRewards.getCategory(
                LGConfigs.REWARDS.rewardsMinesweeper.getStage2()
                    .getCategoryName()),
            cfgRewards.getCategory(
                LGConfigs.REWARDS.rewardsSudoku.getStage2()
                    .getCategoryName()));
        rounds = processStageConfig(
            rounds,
            ...
            cfgRewards.getCategory(
                LGConfigs.REWARDS.rewardsGol.getStage3()
                    .getCategoryName()),
            cfgRewards.getCategory(
                LGConfigs.REWARDS.rewardsMinesweeper.getStage3()
                    .getCategoryName()),
            cfgRewards.getCategory(
                LGConfigs.REWARDS.rewardsSudoku.getStage3()
                    .getCategoryName()));
        processStageConfig(
            rounds,
            ...
            cfgRewards.getCategory(
                LGConfigs.REWARDS.rewardsGol.getStage4()
                    .getCategoryName()),
            cfgRewards.getCategory(
                LGConfigs.REWARDS.rewardsMinesweeper.getStage4()
                    .getCategoryName()),
            cfgRewards.getCategory(
                LGConfigs.REWARDS.rewardsSudoku.getStage4()
                    .getCategoryName()));
    ...
    private static int processStageConfig(int startDigits, LegacyLGConfig.LootStageConfig legacyStage,
        ...
        ConfigCategory catSudokuRewards) {
        int roundsStage2 = Math.max(legacyStage.MinDigitsRequired - startDigits, 1);
        catStage.get(ConfigGOL.Names.ROUNDS)
            .set(roundsStage2);
        catStage.get(ConfigGOL.Names.RANDOMIZE_SEQUENCE)
            .set(legacyStage.RandomizeSequence);
        catStage.get(ConfigGOL.Names.DISPLAY_TIME)
            .set(legacyStage.DisplayTime * 20 / 1000);

        catGolRewards.get(RewardConfig.Names.MIN_ITEMS)
            .set(legacyStage.MinItems);
        catGolRewards.get(RewardConfig.Names.MAX_ITEMS)
            .set(legacyStage.MaxItems);
        catGolRewards.get(RewardConfig.Names.DEFAULT_LOOT_TABLE)
            .set(legacyStage.LootTable);
        catGolRewards.get(RewardConfig.Names.PER_DIM_CONFIGS)
            .set(
                legacyStage.DimensionalLoots.entrySet()
                    .stream()
                    .map(e -> e.getKey() + "|" + e.getValue().LootTable)
                    .toArray(String[]::new));

        catMsRewards.get(RewardConfig.Names.MIN_ITEMS)
            .set(legacyStage.MinItems);
        catMsRewards.get(RewardConfig.Names.MAX_ITEMS)
            .set(legacyStage.MaxItems);
        catMsRewards.get(RewardConfig.Names.DEFAULT_LOOT_TABLE)
            .set(legacyStage.LootTable);
        catMsRewards.get(RewardConfig.Names.PER_DIM_CONFIGS)
            .set(
                legacyStage.DimensionalLoots.entrySet()
                    .stream()
                    .map(e -> e.getKey() + "|" + e.getValue().LootTable)
        ...
        catSudokuRewards.get(RewardConfig.Names.MIN_ITEMS)
            .set(legacyStage.MinItems);
        catSudokuRewards.get(RewardConfig.Names.MAX_ITEMS)
            .set(legacyStage.MaxItems);
        catSudokuRewards.get(RewardConfig.Names.DEFAULT_LOOT_TABLE)
            .set(legacyStage.LootTable);
        catSudokuRewards.get(RewardConfig.Names.PER_DIM_CONFIGS)
            .set(
                legacyStage.DimensionalLoots.entrySet()
                    .stream()
                    .map(e -> e.getKey() + "|" + e.getValue().LootTable)
